...
基于 YOLO11n 的工程施工员安全帽检测项目，提供图片、视频和实时检测功能，支持模型管理、数据集管理、用户管理和数据分析。支持 CPU/GPU 训练、模型导入、参数调节等高级功能。
...
- **图片检测**：
  ...
  - 模型选择：仅显示已发布的模型供选择
  - **置信度调节**：支持动态调整置信度阈值（0-100%）
  - **模型验证**：选择未训练完成的模型时会提示警告

- **视频检测**：
  - 支持视频文件上传（MP4/AVI 格式）
  - 批量处理视频帧，生成检测结果视频
  - 提供检测摘要统计和关键帧展示（仅显示有检测结果的关键帧）
  - **置信度调节**：支持动态调整置信度阈值（0-100%）
  - **检测 FPS 调节**：支持调整检测帧率（1-30 FPS），平衡检测速度和准确性
  - **关键帧统计**：显示原始关键帧数、检测帧数、总检测数等统计信息
  - **工具提示**：鼠标悬停显示每个统计项的含义
  - **模型验证**：仅显示已发布的模型供选择
...
- **实时检测**：
  ...
  - 支持开始/停止检测控制
  - **置信度调节**：支持动态调整置信度阈值（0-100%）
  - **检测 FPS 调节**：支持调整检测帧率（1-30 FPS）
  - **统计重置**：开始检测时自动清空之前的统计数据
  - **模型验证**：仅显示已发布的模型供选择

- **多模型支持**：提供已训练通用安全帽检测模型和定制化训练模型两种选择
- **模型发布机制**：只有已发布的模型才能在检测页面使用

### 模型管理（管理员功能）

- **模型列表**：查看所有可用模型，显示模型类型、描述、训练数据集、基础模型、训练参数、状态、训练结果、创建时间等信息
- **模型创建**：
  - 创建新的自定义模型
  - 配置训练参数：训练数据集、基础模型、迭代次数、批次大小、**训练设备（CPU/GPU）**
  - 创建后不自动开始训练，需要手动点击"训练"按钮
- **模型编辑**：编辑模型名称、类型、描述和训练参数（包括训练设备）
- **模型导入**：支持导入已有的 `.pt` 模型文件
  - 支持通用模型和自定义模型两种类型
  - 默认类型为"通用模型"
  - 导入的通用模型默认状态为"已完成"
- **模型训练**：
  - 支持使用数据集训练自定义模型
  - **训练设备选择**：支持选择 CPU 或 GPU 训练
  - **GPU 检测**：如果选择 GPU 但 GPU 不可用，系统会提示错误并建议使用 CPU
  - 使用保存的训练参数进行训练（无需弹出参数选择对话框）
  - 训练状态实时更新（训练中、训练完成、训练失败）
- **模型发布/取消发布**：发布模型后才能在检测页面使用
- **模型删除**：删除不需要的模型
- **模型详情**：
  - 查看模型基本信息（包括训练设备）
  - 查看性能指标（mAP、精确率、召回率、F1 值）
  - 查看训练数据（损失曲线、性能指标曲线）
- **训练状态**：显示模型状态（待训练、训练中、训练完成、已发布、训练失败）
- **数据同步**：自动检查模型文件是否存在，如果文件缺失会自动更新模型状态
...
- **分布统计**：检测结果分布（饼图）
- **每日统计**：每日检测数量统计（分组柱状图）
  - 按检测类型分组显示：图片检测、视频检测、实时检测
  - 支持查看每种类型的每日检测次数
  - 图例可点击切换显示/隐藏
- **历史记录**：查看历史检测记录
- **类型区分**：检测记录按类型（image、video、realtime）分别存储和统计
...
3. **视频检测**：
   ...
   - 选择已发布的模型（仅显示已发布的模型）
   - 调整置信度阈值（可选，默认 25%）
   - 调整检测 FPS（可选，默认 10 FPS）
   - 等待处理完成
   - 查看检测结果视频和统计摘要（仅显示有检测结果的关键帧）

4. **实时检测**：
   - 进入"实时检测"页面
   - 选择已发布的模型（仅显示已发布的模型）
   - 调整置信度阈值（可选，默认 25%）
   - 调整检测 FPS（可选，默认 5 FPS）
   - 点击"开始检测"（需要允许浏览器访问摄像头，会自动清空之前的统计数据）
   - 查看实时检测结果和统计
   - 点击"停止检测"结束检测（自动保存统计数据到数据库）
...
1. **模型管理**：
   - 创建新模型（自定义模型，配置训练参数包括训练设备）
   - 编辑模型信息（名称、类型、描述、训练参数）
   - 导入已有模型文件（`.pt` 格式，支持通用模型和自定义模型）
   - 查看模型详情和训练数据
   - 训练自定义模型（使用保存的训练参数，支持 CPU/GPU 选择）
   - 发布/取消发布模型（只有已发布的模型才能在检测页面使用）
   - 删除模型
...
4. **数据统计**：
   - 查看系统检测统计
   - 查看趋势图表和分布图表
   - 查看每日检测统计（按图片、视频、实时检测三种类型分组显示）
   - 查看历史检测记录（按类型区分：image、video、realtime）
...
5. **性能优化**：
   - 视频检测处理时间取决于视频长度和分辨率
   - **GPU 训练支持**：系统支持选择 GPU 训练，自动检测 GPU 可用性
   - 如果 GPU 不可用，系统会提示错误并建议使用 CPU
   - 建议使用 GPU 加速（需要安装 CUDA 版本的 PyTorch）
   - 大文件处理可能需要较长时间
   - 数据集上传和验证可能需要一些时间
   - 检测 FPS 可调节，平衡检测速度和准确性
...
### v1.2.0 (最新)
- ✅ **模型导入功能**：支持导入已有的 `.pt` 模型文件
- ✅ **模型编辑功能**：合并创建和编辑对话框，统一操作界面
- ✅ **训练设备选择**：支持选择 CPU 或 GPU 训练，自动检测 GPU 可用性
- ✅ **模型发布机制**：只有已发布的模型才能在检测页面使用
- ✅ **置信度调节**：图片、视频、实时检测支持动态调整置信度阈值
- ✅ **检测 FPS 调节**：视频和实时检测支持调整检测帧率
- ✅ **检测类型区分**：数据库按 image、video、realtime 三种类型分别存储
- ✅ **统计图表优化**：每日统计按检测类型分组显示（图片、视频、实时）
- ✅ **视频检测优化**：仅显示有检测结果的关键帧，增加原始关键帧统计
- ✅ **实时检测优化**：开始检测时自动清空统计数据，停止时保存到数据库
- ✅ **模型信息完善**：模型详情页面显示训练设备信息
- ✅ **数据同步机制**：自动检查模型文件是否存在，同步数据库状态
- ✅ **错误处理增强**：GPU 训练失败时提供详细错误提示
